	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if (opr.equals("addComment")) {// 添加评论
				...
				topics.put(2, 5);
				topics.put(5, 5);
				List<List<News>> latests = newsService.findLatestNewsByTid(topics);
				...
				for (int i = 0;;) {
					...
					newsJSON.append("\"nauthor\":\"" + news.getNauthor().replace("\"", "&quot;") + "\"}");
				...
				// response.sendRedirect(contextPath+"/newspages/showNews.jsp");
				...
				try {
					...
					if (result == 0) {
						...
						out.print("location.href=\"" + contextPath + "/util/news?opr=list\";");
						out.print("</script>");
					} else {
						out.print("<script type=\"text/javascript\">");
				...
				try {
					...
					if (result == 0) {
						...
						out.print("location.href=\"" + contextPath + "/util/news?opr=toModifyNews&nid=" + nid + "\";");
						out.print("</script>");
					} else {
						out.print("<script type=\"text/javascript\">");
					...
					try {
						...
						if (isUnallowedType) {
							...
							if (opr.equals("addNews"))
								out.print("location.href=\"" + contextPath + "/util/news?opr=toAddNews\";");
							else
								out.print("location.href=\"" + contextPath + "/util/news?opr=toModifyNews&nid="
										+ news.getNid() + "\";");
							out.print("</script>");
						} else {
